...
describe('DELETE /clubs/:clubId/teams/:teamId', () => {
  it('deletes team and removes assignments', async () => {
    ...
    expect(m.teamAssignment.deleteMany).toHaveBeenCalled()
  })
})
...
describe('DELETE /clubs/:clubId/teams/:teamId/squad/:assignmentId', () => {
  it('soft-deletes by setting removedAt', async () => {
    m.teamAssignment.findFirst.mockResolvedValue({ id: 'a-1', teamId: TEAM_ID })
    m.teamAssignment.update.mockResolvedValue({})
    const app = await buildApp()
    const res = await app.inject({ method: 'DELETE', url: `/clubs/${CLUB}/teams/${TEAM_ID}/squad/a-1`, headers: { authorization: token() } })
    expect(res.statusCode).toBe(204)
