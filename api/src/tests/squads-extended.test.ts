...
describe('DELETE /clubs/:clubId/teams/:teamId', () => {
  ...
  it('deletes team assignments before deleting team', async () => {
    m.team.findFirst.mockResolvedValue({ id: TEAM_ID })
    m.teamAssignment.deleteMany.mockResolvedValue({ count: 3 })
    m.team.delete.mockResolvedValue({})
    const app = await buildApp()
    const res = await app.inject({ method: 'DELETE', url: `/clubs/${CLUB}/teams/${TEAM_ID}`, headers: { authorization: token() } })
...
describe('POST /clubs/:clubId/teams/:teamId/squad', () => {
  it('returns 409 when member already on team this season', async () => {
    ...
    m.teamAssignment.findFirst.mockResolvedValue({ id: ASSIGNMENT_ID })
    const app = await buildApp()
    const res = await app.inject({
  ...
  it('creates assignment and returns 201', async () => {
    ...
    m.teamAssignment.create.mockResolvedValue({ id: ASSIGNMENT_ID, clubMember: { user: {} } })
    const app = await buildApp()
    const res = await app.inject({
      method: 'POST', url: `/clubs/${CLUB}/teams/${TEAM_ID}/squad`,
      headers: { authorization: token(), 'content-type': 'application/json' },
...
describe('DELETE /clubs/:clubId/teams/:teamId/squad/:assignmentId (soft delete)', () => {
  ...
  it('soft deletes assignment by setting removedAt', async () => {
    m.teamAssignment.findFirst.mockResolvedValue({ id: ASSIGNMENT_ID })
